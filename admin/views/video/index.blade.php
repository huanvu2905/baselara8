...
@section('title', __('Quản lý video'))
...
<div class="row">
    <div class="card border border-primary">
        <div class="card-body">
            ...
            <a href="{{ route('admin.video.create') }}" class="btn btn-primary waves-effect waves-light">
            ...
            <div class="table-responsive">
                <table class="table mb-0">
                    ...
                    <tbody>
                        ...
                        <tr class="align-middle {{ $loop->iteration % 2 == 0 ? 'table-primary' : '' }}" id="{{ \Arr::get($video, 'id') }}">
                            <td>{{ $video->id }}</td>
                            @php
                                $trans = \Arr::get($video, 'videoTranslations')->where('lang', 'vi')->first();
                                $category = \Arr::get($video, 'category');
                                $category->load(['videoCategoryTranslations']);
                                $categoryTrans = \Arr::get($category, 'videoCategoryTranslations')->where('lang', 'vi')->first();
                            @endphp
                            <td><a href="javascript:;">{{ \Arr::get($trans, 'name') }}</a></td>
                            <td>{{ \Arr::get($categoryTrans, 'name') ?? \Arr::get($category, 'name') }}</td>
                            <td>{{ \Arr::get($video, 'views') }}</td>
                            <td>{{ \Arr::get($video, 'updated_at') }}</td>
                            <td>{{ ($video->active==1) ? 'Active' : 'Deactive' }}</td>
                                ...
                                <div>
                                    <div class="btn-group btn-group-example" role="group">
                                        ...
                                        <button
                                            ...
                                            data-url="{{ route('admin.video.destroy', $video->id) }}"
                                        >
